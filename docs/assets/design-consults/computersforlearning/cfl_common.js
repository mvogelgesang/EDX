...
function hlpOpen(trnNumb) {
  ...
  winFeatures =
    "toolbar=yes,titlebar=yes,locationbar=no,status=yes,scrollbars=yes,resizable=yes";
...
var imgSpace = '<img src="hp_spacer.gif" width="4" alt="">';
...
function timeOut() {
  winFeatures =
    "toolbar=yes,titlebar=yes,locationbar=no,status=yes,scrollbars=yes,resizable=yes,alwaysRaised=yes";
  if (scSignIn != "N")
    timeOutWin = window.open(
      "/htm/cfltimeout.htm",
      "Timeout",
      "width=550,height=200,top=160,left=200",
      "winFeatures"
    );
  if (scSignIn != "N") {
    if (timeOutWin) {
...
function saveData() {
  if (scSignIn != "N") {
    if (timeOutWin) timeOutWin.close();
  }
  if (scSignIn == "Y") {
