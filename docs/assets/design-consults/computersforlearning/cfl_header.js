...
var pubHost = "";
if (
  window.location.host == "computersforlearning.gov" ||
  window.location.host == "www.cfl.gov" ||
  window.location.host == "computers4learning.gov"
)
  pubHost = "https://ComputersforLearning.gov";
else pubHost = "https://cflpractice.fas.gsa.gov";
...
function selLnk(opt) {
  if (scSignIn == "Y") document.forms[0].scSelState[0].selected = true;
  document.forms[0].scSelLink.value = opt;
  if (opt == "GS")
    document.forms[0].scGSName.value = document.forms[0].scGSName.value.replace(
      /^\s+/,
      ""
    );
  document.forms[0].submit();
...
function uCGSName(fld) {
  if (fld.value != "Search Entire Site") fld.value = fld.value.toUpperCase();
}
function nw1(pgname) {
...
function nwHP(htmPage) {
  ...
  if (htmPage == "privacy") loc = "https://www.gsa.gov/portal/content/116609";
  else if (htmPage == "usa") loc = "https://www.usa.gov";
  if (scSignIn == "Y") newWindow = window.open(loc, "CFLPage", "");
...
function confmHome() {
  if (scSignIn == "Y") {
    if (
      confirm(
        "Going Home, your current session will be closed \n \n       and you need to Sign in again"
      )
    )
      return true;
    else return false;
...
function bldGSOptn() {
  ...
  var sel = "selected";
  if (gSOptn == "2") sO2 = sel;
  else if (gSOptn == "3") sO3 = sel;
  else sO1 = sel;
  srchOptnLn =
    '<label for="scGSOptnId"></label><select name=scGSOptn id="scGSOptnId" size=1 class=fontLB9>' +
    '<option value="1"' +
    sO1 +
    " >All Words</option>" +
    '<option value="2"' +
...
function bldState() {
  stateLn =
    '<label for="StateList">or Browse by State: </label>' +
...
function hdrFlag() {
  ...
  if (scSignIn == "Y") {
    htmSign_in =
      ...
      ' title="CFL Sign out">' +
      '<img src="sign_out_button.gif" alt="Sign out" border="0" align="top"></a>';
  } else if (scSignIn == "N") {
    //htmSign_in='<a href="javascript:openCflLog()" title="Sign in">'
    //+'<img src="sign_in_button.gif" alt="Sign in" border="0" align="top"></a>';
  } else {
    scSignIn = " ";
  }
  ...
  htmBanner =
    ...
    '<img src="../../assets/design-consults/computersforlearning/cfl_flag_top.jpg" border="0" alt="Image of Flag" id="hp_Top_Flag">' +
    "<a href=" +
    pubHost +
    ...
    '<div id="hp_Flag_Image"><img src="../../assets/design-consults/computersforlearning/cfl_flag_left.jpg" alt="General Services Administration, Computers For Learning" border="0"></div>' +
    /* right_Flag sets the float property to left which moves flag_Right.gif to right */
    '<div id="right_Flag"><div align="right">' +
    htmSign_in +
    '<img src="../../assets/design-consults/computersforlearning/cfl_flag_right.gif" border="0" alt="United States Flag"></a></div>' +
    "</div>" +
    "</div>" +
...
function hdrTabs() {
  ...
  if (curTabCat == "01001") cat01_001 = curId;
  else if (curTabCat == "01002") cat01_002 = curId;
  else if (curTabCat == "01003") cat01_003 = curId;
  else if (curTabCat == "01004") cat01_004 = curId;
  else if (curTabCat == "02001") cat02_001 = curId;
  else if (curTabCat == "02002") cat02_002 = curId;
  else if (curTabCat == "02003") cat02_003 = curId;
  else if (curTabCat == "02004") cat02_004 = curId;
  else if (curTabCat == "02005") cat02_005 = curId;
  else if (curTabCat == "02006") cat02_006 = curId;
  else if (curTabCat == "02007") cat02_007 = curId;
  else if (curTabCat == "02008") cat02_008 = curId;
  else if (curTabCat == "02009") cat02_009 = curId;
  else cat00_000 = curId;
...
function hdrSrchBar() {
  ...
  var srchLn1 = "",
    ...
    srchLn3 = "";
  srchLn1 =
    '<table align="left" id=hp_hdr_bar border="1" bordercolor="#ffffff" cellpadding="0" cellspacing="0"' +
    'marginheight="0" marginwidth="0" width="100%" summary="Table contains Search Options">' +
    "<tr>" +
    '<td><img src="hp_spacer.gif" width="10" height="1" alt=""></td>';
  srchLn2 =
    '<td nowrap width=40%><input id="scGSNameId" onclick="blankGSName(this)" onblur="uCGSName(this),trim(this)" name="scGSName" size="25%" maxlength=25 value="Search Entire Site">&nbsp;' +
    ...
    ')"' +
    ">";
  srchLn3 = '<td width=30%><img src="hp_spacer.gif" alt=""></td>';
  //+'<td nowrap width=0%>&nbsp;</td>';

  var srchLn3a =
    '<td width=10%><a href="javascript:selLnk(' +
    "'UU'" +
  ...
  var srchLn3c = '<td width=15%><img src="hp_spacer.gif" alt=""></td>';
  //+'<td nowrap width=0%>&nbsp;</td>';
...
function ftrLn() {
  htmln10 =
    ...
    '<a href=javascript:nwHP("usa")> <img src="../../assets/design-consults/computersforlearning/usagov_logo.jpg" border="0" alt="USA.gov is the U.S. government\u0027s official web portal to all federal, state and local government web resources and services."></a></div>';
  htmln30 =
  ...
  document.getElementsByName("cflwel")[0].innerHTML += htmln10 + htmln30;
}
