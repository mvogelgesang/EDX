...
base(AIRTABLE_TABLES.websites)
  ...
  .eachPage(
    ...
    function done(err:any) {
      ...
      mkdir('public/data/archive', { recursive: true }, (err:any) => {
        if (err) {
          console.error(err);
          return;
        }
      ...
      writeFile(
        `public/data/archive/websites_${formattedDate()}.json`,
        JSON.stringify(data),
        (err: any) => {
          if (err) {
            console.error(err);
      ...
      writeFile(`public/data/websites.json`, JSON.stringify(data), (err:any) => {
        if (err) {
          console.error(err);
          return;
        }
